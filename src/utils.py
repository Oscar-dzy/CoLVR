...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="vsp-spatial-reasoning", choices=[
        "vsp-spatial-reasoning", 
        "vsp-spatial-planning-vanilla-sft", "vsp-spatial-planning", "vsp-spatial-planning-progressive", 
        "jigsaw-vanilla-sft", "jigsaw", "jigsaw-mirage", "jigsaw-progressive", 
        "tertis-vanilla-sft", "tertis", "tertis-mirage", "tertis-progressive"
    ])

    parser.add_argument("--latent_size", type=int, default=8)
    parser.add_argument("--compress_strategy", type=str, default='average', choices=['average'])
    parser.add_argument("--progressive_strategy", type=str, default='meanPool', choices=['meanPool'])
    parser.add_argument("--proj_strategy", type=str, default='None', choices=['None', 'split_dim', 'attention_split'], help="Different strategies for increasing the dimensionality of hidden states")
    parser.add_argument("--contrastive_strategy", type=str, default='None', help="Different strategies for increasing the dimensionality of hidden states")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1)  # Gradient accumulation steps
    parser.add_argument("--max_gpu_memory", type=float, default=1.0)  # Gradient accumulation steps
    ...
    parser.add_argument("--output_results_dir", type=str, default='save evaluate results')
    parser.add_argument("--output_response", action="store_true")
    ...
    parser.add_argument(
        "--eval_cot",
        action="store_true",
        help="CoT evaluation: Takes the last boxed token from the model output as the predicted answer (preceded by inference); can be used with `-max_new_tokens` to increase the generation length.",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
