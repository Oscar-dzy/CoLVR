...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        latent_num = 0
        latent_end_num = 0
        # Fixed latent tokens: set TRANSFORMERS_LATENT_FIXED_INFERENCE=1 to replace premature <|latent_end|> with pad
        # until TRANSFORMERS_MAX_LATENT_LEN latent steps (default 8). Override length with TRANSFORMERS_MAX_LATENT_LEN.
        latent_fixed_infer = os.environ.get("TRANSFORMERS_LATENT_FIXED_INFERENCE", "").lower() in ("1", "true", "yes", "on")
        try:
            MAX_LATENT_LEN = int(os.environ.get("TRANSFORMERS_MAX_LATENT_LEN", "8"))
        except ValueError:
            MAX_LATENT_LEN = 8
        ...
        while self._has_unfinished_sequences(
            ...
            if not in_latent_mode:              
                ...
                if latent_start:
                    ...
                    # The inputs_embeds here should correspond to the embeddings generated at the current step
            ...
            for i, token_str in enumerate(next_token_strs):
                if token_str == latent_start_idx:
                    ...
                    if latent_fixed_infer and latent_num < MAX_LATENT_LEN:
            ...
            if in_latent_mode and not latent_end and not latent_start:
                ...
                if latent_num > MAX_LATENT_LEN:
                    latent_end = True
                    in_latent_mode = False
                    next_tokens[:, None] = latent_end_idx  # force output <latent> 
            ...
            if in_latent_mode and not latent_start:
                ...
                # Optional: perturb latent-step embeddings (config.latent_embed_noise_std > 0), set from test/eval scripts
                noise_std = getattr(self.config, "latent_embed_noise_std", 0.0)
                if noise_std is not None and float(noise_std) > 0:
                    std = float(noise_std)
                    noise = torch.randn_like(cur_latent_embeds, dtype=torch.float32) * std
                    cur_latent_embeds = (cur_latent_embeds.float() + noise).to(dtype=cur_latent_embeds.dtype)
                inputs_embeds = cur_latent_embeds  # 将当前的 hidden states 传给下一次的输入
